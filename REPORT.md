...
This report will present 4-phase agent that **improved mutation kill-rate by an avg of ~11.7pp** over single-shot LLM prompting on 12 functions, with 3 functions exceeding +20pp **at total API cost ~$0.13**
**What this agent 'becomes'?** Function-specific prompting + iteration policy => `specialization_record.yaml` per run. Each record contains agent chosen test category, candidate patterns, identified blind spots and the iteration trajectory => specialization context
...
- **Mellum-style context composition for code completion** - **EnsembleAI ** experience inspired. The closest to JetBrains own tool **Kill reason:** completion benchmarks are noisy => evaluation is the single most important step, that could've eaten days with no rollback after failure.
...
**Hypothesis:** After commiting 1 day to research => valuable articles & repos & benchmarks I have found out that **Pynguin Generated vs Human-Written** were leaning towards **generated**, while **Pynguin vs Naive** has *mixed results* with its nuances. But here generated is search algorithm that derives test cases from source code.
...
**Memorization-as-generalization** - instead of asking "does the agent help", it should be whether LLM solving problem or pattern-matching to training data? gpt-4o-mini has seen tens of thousands of linked-list and BST test suites. Its "specialization" on those programs might be retrieval, not reasoning. **Solution:** Internal (handwritten) tier of the benchmark with pure logic, state machines, in-file dependencies. if the agent helps on those *and* on textbook code, it's reasoning.

**The "fancy prompt engineering" risk** - multi-phase agent thats just 3 prompts in a loop was shallow, not a research submission. I needed some *innovation* that would make STEM concept *measurably different* than a single-shot call could. **Solution:** Clear reproducible environment + artifacts + smoke tests => naive variance is wide and stem variance is tight

**Benchmark contamination** - wanted to pursue with synthetic function generation + using public code repos. Risking to make the naive baseline artificially strong because the agent training and test distribution would overlap **Solution:** Just abandon the idea + as above implement handwritten functions
...
**Observation:** Not a primary comparison metric, because these tests were produced **by a search algorithm, not an LLM and with a different pipeline**. Interestingly **better than human-written ones** and **not better or worse than stem (and vice versa, 4.1 Comparison + Notes)** => **proves further the value of STEM Concept**
...
![Stem Agent Architecture](./images/architecture_uml.png)

### 3.5 Scope Decisions

- N=3 runs for stem agent (vs N=5 for naive). Due to Cost + Time Constraints
...
**Interpretation:** 10 / 12 functions improved by >1% & 0 regressed & mean delta across the comparison set is **+11.7pp**. Three functions improved by >20% (validate_date, parse_log_line, identifier1) - naive prompting was unstable to begin with (`22<std<36`)
...
**Total experiment cost:** ~$0.13 across all 33 stem runs & 70 naive runs at gpt-4o-mini pricing

## 5. What surprised & failed me

### 5.1 Metric Inversion

* Cosmic-ray `cr-rate` reports survival => not kill rate. This invalidated several days of comparison data and required recomputation. **Lesson:** double-check cli output
...
- **Spec Ambiguity:** `apply_discount` initially scored 0% during the pilot phase not because the tests were bad, but because the spec said **"raises ValueError on None" while the function raised TypeError** The LLM correctly followed the spec => the function contradicted itself
...
[static.py](./stem_agent/static.py) has var `IS_DEMO_MODE` When True => project will run both pipelines on `validate_date` with N=1, MAX_ITER=1. **ETA:** ~2-5 minutes for naive, ~5-10 minutes for stem. Verifies installation and creates artifacts in results/demos.
