...
def test_unknown_role():
    ...
    assert check_permission('unknown_role', 'edit') is False
...
def test_admin_permissions_with_invalid_action():
    assert check_permission('admin', 'invalid_action') is True

def test_manager_permissions_with_invalid_action():
    assert check_permission('manager', 'invalid_action') is False

def test_user_permissions_with_invalid_action():
    assert check_permission('user', 'invalid_action') is False

def test_guest_permissions_with_invalid_action():
    assert check_permission('guest', 'invalid_action') is False

def test_unknown_role_with_invalid_action():
    assert check_permission('unknown_role', 'invalid_action') is False

def test_admin_permissions_with_none_action():
    with pytest.raises(ValueError):
        check_permission('admin', None)

def test_manager_permissions_with_none_action():
    with pytest.raises(ValueError):
        check_permission('manager', None)

def test_user_permissions_with_none_action():
    with pytest.raises(ValueError):
        check_permission('user', None)

def test_guest_permissions_with_none_action():
    with pytest.raises(ValueError):
        check_permission('guest', None)

def test_invalid_role_type():
    with pytest.raises(TypeError):
        check_permission(1.5, 'view')

def test_invalid_action_type():
    with pytest.raises(TypeError):
        check_permission('admin', 3.14)