...
def test_input_validation():
    ...
    with pytest.raises(TypeError):
        parse_user_id_from_string(12.34)
    with pytest.raises(TypeError):
        parse_user_id_from_string([])

def test_fallback_digit_collection():
    assert parse_user_id_from_string('abc123def456') == 123456
    assert parse_user_id_from_string('mixed123abc456') == 123456
...
def test_performance():
    long_string = 'a' * 10000 + '12345' + 'b' * 10000
    assert parse_user_id_from_string(long_string) == 12345