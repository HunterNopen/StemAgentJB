...
def test_postfix_eval():
    ...
    assert stack1.postfix_eval("2 3 4 + *") == 14
    assert stack1.postfix_eval("5 1 2 + 4 * + 3 -") == 14
    assert stack1.postfix_eval("3 4 2 * 1 5 - 2 3 ^ ^ / +") == 3.0001220703125

def test_postfix_eval_invalid_tokens():
    with pytest.raises(ValueError):
...
def test_postfix_eval_insufficient_operands():
    with pytest.raises(ValueError):
        stack1.postfix_eval("+")
    with pytest.raises(ValueError):
        stack1.postfix_eval("2 3 + 4 - +")  # Extra operator

def test_postfix_eval_division_by_zero():
    with pytest.raises(ZeroDivisionError):
        stack1.postfix_eval("4 0 /")