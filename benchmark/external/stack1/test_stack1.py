...
import unittest
from stack1 import Stack, check_parenthesis, postfix_eval
...
class TestApplications(unittest.TestCase):
    ...
    def test_parenthesis(self):
        ...
        self.assertTrue(check_parenthesis(well_formed))
        self.assertFalse(check_parenthesis(not_well_formed))
        self.assertTrue(check_parenthesis(well_formed_with_text))
        self.assertFalse(check_parenthesis(not_well_formed_with_text))
    ...
    def test_postfix_eval(self):
        ...
        self.assertEqual(88, postfix_eval(eighty_eight))
        with self.assertRaises(ValueError): 
            postfix_eval(too_many_operands)

        with self.assertRaises(ValueError):
            postfix_eval(too_many_operators)
